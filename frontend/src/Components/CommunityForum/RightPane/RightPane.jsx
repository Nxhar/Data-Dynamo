...
function RightPane({user, context}) {

  const [message, setMessage] = useState('')
  const [messages, setMessages] = useState([
    {
      'type' : 'aiMessage',
      'content': "Hi! I'm an AI assistant to help with your needs. You can ask me anything you like!"
    }
  ])
  ...
  const handlePostContext = async () => {
    ...
    const prompt_template = `
       You are an AI assistant that speaks about a certain discussion and answers the queries in the discussion if any in at most 100 words.
       You must end each answer with the characters </end> 
       DO NOT FORGET TO PUT </end> with each answer.
       Discussion : ${context}
       Answer:
    `;  
    setLoadingResponse(true)
  ...
  return (
    ...
    <div className="outerRightLayer">
      ...
      <div className="bottomRightLayer" onSubmit={ async (e)=> {e.preventDefault(); await handlePostChatMessage() }}>
        ...
        {context!=='' && (<div className='getInsights' onClick={handlePostContext}>Get Insights</div>)}
